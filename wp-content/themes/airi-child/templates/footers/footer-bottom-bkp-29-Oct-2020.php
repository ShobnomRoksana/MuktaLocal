...
<div class="cart-flyout">
    <div class="cart-flyout--inner">
        ...
        <div class="cart-flyout__content" style="padding-top: 0px;">
            <div class="cart-flyout__heading row"><div style="font-family: 'Raleway', sans-serif;font-weight: 600;font-size: 1.8rem;text-align: center;text-transform: uppercase;letter-spacing: 0.16rem;padding-top: 6px;padding-bottom: 15px;border-bottom: 3px solid #f0f0f0;"><?php echo esc_html_x('Shopping Bag', 'front-view', 'airi') ?></div><div><a href="javascript:;" class="btn-close-cart"><i class="dl-icon-close"></i></a></div></div>
...
        if($j(this).is(":checked")) {
  // var hostname = $j('<a>').prop('href', url).prop('hostname');
  // alert(hostname);
...
            $j("#shipping_country").val($j('#billing_country').val());
            $j("[name='shipping_phone']").val($j("[name='billing_phone']").val());
            $j("[name='shipping_email']").val($j("[name='billing_email']").val());
        }
        else {
...
            $j("#shipping_country").val('');
            $j("[name='shipping_phone']").val('');
            $j("[name='shipping_email']").val('');
        }
    });
